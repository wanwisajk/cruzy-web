...
export default function LeaveDashboard({ data, currentBranch }) {
      ...
      .map((employee) => {
        ...
        return {
          ...
          summary: {
            ...
            annualQuota: Number(
              leaveBalance.annualQuota ?? leaveBalance.annual_quota ?? 13,
            ),
            vacationUsed: totals.vacation,
            vacationQuota: Number(
              leaveBalance.vacationQuota ?? leaveBalance.vacation_quota ?? 5,
            ),
  ...
  const months = useMemo(() => {
    ...
    return Array.from(monthsMap.entries()).map(([value, label]) => ({
      value,
      label,
    }));
  }, [leaves]);
  ...
  return (
    <div className="content p-6">
        ...
        <>
          ...
          <div className="mt-4">
            <LeaveHistoryTable
              rows={historyLeaves}
              employees={tableEmployees}
              onView={handleViewApprovedSummary}
            />
          </div>
      ...
      <div
        className={`overlay ${confirmationOpen ? "open" : ""}`}
        onClick={(event) =>
          event.target === event.currentTarget && closeConfirmation()
        }
      >
        <div className="modal">
          ...
          <div className="m-body space-y-3">
            ...
            <p className="text-sm text-slate-500">
              คำขอนี้จะถูกลบออกจากระบบและไม่สามารถกู้คืนได้
            </p>
          </div>
