...
export default function ScheduleDashboard({ data, setData, user, currentBranch, from, to, toast }) {
  ...
  async function addToSchedule(branchId, date, empId) {
    ...
    try {
      ...
      await assignSchedule({ branchId, date, employeeId: empId, shiftStart: shift.start, shiftEnd: shift.end });
    ...
    for (const date of days) {
      for (const branch of branches) {
        ...
        while (existing.length < need) {
          const rec = recommendedEmployees(scheduleData, user, branch.id, date, { from, to })[0];
          if (!rec) break;
