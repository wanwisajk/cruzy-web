...
function SalesEditorModal({ data, sale, mode, onClose, onSaved, onUpsert, onRefresh }) {
  ...
  async function save() {
    ...
    try {
      ...
      const result = sale ? await api.updateSale(sale.id, payload) : await api.createSale(payload);
      const saved = normalizeSale(result.data);
      if (newImages.length) {
        const created = await api.createAttachments(newImages.map((fileUrl) => ({
          entityType: 'sale',
          entityId: saved.id,
          fileUrl
        })));
...
function DepositEditorModal({ data, deposit, mode, onClose, onSaved, onUpsert, onRefresh }) {
  ...
  async function save() {
    ...
    try {
      ...
      const result = deposit ? await api.updateCashDeposit(deposit.id, payload) : await api.createCashDeposit(payload);
      const saved = normalizeDeposit(result.data);
      if (slipImage && (slipChanged || !deposit)) {
        const created = await api.createAttachments([{
...
function AccountFormModal({ account, mode = 'create', onClose, onSaved, onCreated, onUpdated }) {
  ...
  async function save() {
    setSaving(true);
    try {
      const result = account ? await api.updateBankAccount(account.id, form) : await api.createBankAccount(form);
...
export function SalesDashboardPage({ data, user, currentBranch, from, to }) {
  ...
  async function verifyDeposit(deposit) {
    try {
      await api.updateCashDeposit(deposit.id, {
        status: 'verified',
  ...
  async function toggleBankAccount(account) {
    try {
      await api.updateBankAccount(account.id, { isActive: !account.active });
